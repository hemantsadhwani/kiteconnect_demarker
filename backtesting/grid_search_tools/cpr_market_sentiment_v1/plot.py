...
def main():
    # Load config to get DATE_MAPPINGS and STRIKE_MODE_SETTINGS
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_path = os.path.join(script_dir, 'config.yaml')
    date_mappings = {}
    config = {}
...
Examples:
  ...
  python plot.py --input path/to/file.csv  # Use custom input file
    ...
    # plot.py is always run standalone — config.yaml is the authority.
    ...
    def _resolve_data_dir():
        ...
        if not mode:
            try:
                with open(os.path.join(backtesting_dir, 'backtesting_config.yaml'), 'r') as _f:
                    _bt = yaml.safe_load(_f) or {}
                mode = _bt.get('STRIKE_MODE')
            except Exception:
                pass
        mode = mode or 'ST50'
