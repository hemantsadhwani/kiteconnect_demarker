...
This tool performs grid search optimization for Entry2 STOP_LOSS_PRICE_THRESHOLD
and STOP_LOSS_PERCENT in backtesting_config.yaml. It follows a standardized workflow:

1. Establish baseline from current configuration
2. For each combination:
   - Update backtesting_config.yaml
...
Usage:
    python sl_grid_search.py              # Full grid search
    python sl_grid_search.py --test      # Test mode (4 random combinations)
...
# -----------------------------------------------------------------------------
# Base path: backtesting directory. Change this to run against a different project.
# -----------------------------------------------------------------------------
BACKTESTING_BASE_PATH = Path(r"C:\Users\Hemant\OneDrive\Documents\Projects\kiteconnect_demarker\backtesting")
...
class StopLossGridSearch:
    ...
    def __init__(self, config_path=None):
        ...
        self.backtesting_config_backup = None
        
        logger.info(f"Initialized Stop Loss Grid Search")
    ...
    def _load_config(self, config_path):
        ...
        try:
            ...
            defaults = {
                'GRID_SEARCH': {
                    'STOP_LOSS_PRICE_THRESHOLD_HIGH': {'MIN': 80, 'MAX': 180, 'STEP': 5},
                    'STOP_LOSS_PRICE_THRESHOLD_LOW': {'MIN': 20, 'MAX': 80, 'STEP': 5},
                    'STOP_LOSS_PERCENT_ABOVE_THRESHOLD': {'MIN': 5.0, 'MAX': 7.0, 'STEP': 0.5},
                    'STOP_LOSS_PERCENT_BETWEEN_THRESHOLD': {'MIN': 6.5, 'MAX': 8.0, 'STEP': 0.5},
                    'STOP_LOSS_PERCENT_BELOW_THRESHOLD': {'MIN': 7.0, 'MAX': 8.5, 'STEP': 0.5}
                },
                'OPTIMIZATION': {
    ...
    def load_backtesting_config(self):
        """Load backtesting configuration"""
        try:
            with open(self.backtesting_config_path, 'r') as f:
                return yaml.safe_load(f)
        except Exception as e:
            logger.error(f"Error loading backtesting config: {e}")
            return None
    
    def save_backtesting_config(self, config):
        """Save backtesting configuration"""
        try:
            with open(self.backtesting_config_path, 'w') as f:
                yaml.dump(config, f, default_flow_style=False, sort_keys=False)
            return True
        except Exception as e:
            logger.error(f"Error saving backtesting config: {e}")
            return False
    
    def backup_backtesting_config(self):
        """Create backup of backtesting configuration"""
        try:
            config = self.load_backtesting_config()
            if config is None:
                return False
            
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.backtesting_config_backup = self.results_dir / f"backtesting_config_backup_{timestamp}.yaml"
            
            with open(self.backtesting_config_backup, 'w') as f:
                yaml.dump(config, f, default_flow_style=False, sort_keys=False)
            
            logger.info(f"Backtesting config backed up to: {self.backtesting_config_backup}")
            return True
        except Exception as e:
            logger.error(f"Error backing up backtesting config: {e}")
            return False
    
    def restore_backtesting_config(self):
        """Restore backtesting configuration from backup"""
        if not self.restore_config:
            logger.info("Skipping config restoration (RESTORE_CONFIG=false)")
            return False
        
        if self.backtesting_config_backup is None or not self.backtesting_config_backup.exists():
            logger.warning("No backtesting config backup found to restore")
            return False
        
        try:
            with open(self.backtesting_config_backup, 'r') as f:
                config = yaml.safe_load(f)
            
            with open(self.backtesting_config_path, 'w') as f:
                yaml.dump(config, f, default_flow_style=False, sort_keys=False)
            
            logger.info(f"Backtesting config restored from: {self.backtesting_config_backup}")
            return True
        except Exception as e:
            logger.error(f"Error restoring backtesting config: {e}")
            return False
    ...
    def cleanup_backup_files(self, cleanup_old=True):
        ...
        try:
            ...
            backup_pattern = "backtesting_config_backup_*.yaml"
            ...
            if self.backtesting_config_backup and self.backtesting_config_backup.exists():
                try:
                    self.backtesting_config_backup.unlink()
                    deleted_count += 1
                    logger.info(f"Deleted current backup file: {self.backtesting_config_backup.name}")
                except Exception as e:
                    logger.warning(f"Could not delete current backup {self.backtesting_config_backup.name}: {e}")
            
            # Clean up old backup files from previous runs (if enabled)
            if cleanup_old:
                for backup_file in backup_files:
                    # Skip if it's the current backup (already deleted above)
                    if self.backtesting_config_backup and backup_file == self.backtesting_config_backup:
                        continue
    ...
    def update_stop_loss_parameters(self, threshold_high, threshold_low,
                                   above_percent, between_percent, below_percent):
        """Update Entry2 stop loss parameters in backtesting_config.yaml"""
        config = self.load_backtesting_config()
        if config is None:
            return False
        
        # Ensure ENTRY2 section exists
        if 'ENTRY2' not in config:
            config['ENTRY2'] = {}
        
        # Update STOP_LOSS_PRICE_THRESHOLD (list format: [high, low])
        config['ENTRY2']['STOP_LOSS_PRICE_THRESHOLD'] = [threshold_high, threshold_low]
        
        # Ensure STOP_LOSS_PERCENT section exists
        if 'STOP_LOSS_PERCENT' not in config['ENTRY2']:
            config['ENTRY2']['STOP_LOSS_PERCENT'] = {}
        
        # Update stop loss percentages
        config['ENTRY2']['STOP_LOSS_PERCENT']['ABOVE_THRESHOLD'] = above_percent
        config['ENTRY2']['STOP_LOSS_PERCENT']['BETWEEN_THRESHOLD'] = between_percent
        config['ENTRY2']['STOP_LOSS_PERCENT']['BELOW_THRESHOLD'] = below_percent
        
        return self.save_backtesting_config(config)
    
    def run_indicators(self):
        """Run run_indicators.py - Not needed for stop loss grid search"""
        # Stop loss parameters don't affect indicator calculations,
        # so we skip this step to save time
        logger.debug("Skipping run_indicators.py (not needed for stop loss parameters)")
        return True
    ...
    def establish_baseline(self):
        ...
        # Get current stop loss parameters
        config = self.load_backtesting_config()
        if config is None:
            logger.error("Failed to load backtesting config")
            return False
        
        entry2_config = config.get('ENTRY2', {})
        threshold_list = entry2_config.get('STOP_LOSS_PRICE_THRESHOLD', [120, 70])
        sl_percent = entry2_config.get('STOP_LOSS_PERCENT', {})
        
        threshold_high = threshold_list[0] if len(threshold_list) > 0 else 120
        threshold_low = threshold_list[1] if len(threshold_list) > 1 else 70
        above_percent = sl_percent.get('ABOVE_THRESHOLD', 6.0)
        between_percent = sl_percent.get('BETWEEN_THRESHOLD', 7.5)
        below_percent = sl_percent.get('BELOW_THRESHOLD', 8.0)
        
        logger.info(f"Current configuration:")
        logger.info(f"  STOP_LOSS_PRICE_THRESHOLD: [{threshold_high}, {threshold_low}]")
        logger.info(f"  STOP_LOSS_PERCENT.ABOVE_THRESHOLD: {above_percent}")
        logger.info(f"  STOP_LOSS_PERCENT.BETWEEN_THRESHOLD: {between_percent}")
        logger.info(f"  STOP_LOSS_PERCENT.BELOW_THRESHOLD: {below_percent}")
        logger.info("")
        logger.info("Running baseline workflow...")
        logger.info("Note: Skipping run_indicators.py (stop loss parameters don't affect indicators)")
        
        # Run workflow (indicators not needed for stop loss parameters)
        ...
        self.baseline_metrics['threshold_high'] = threshold_high
        self.baseline_metrics['threshold_low'] = threshold_low
        self.baseline_metrics['above_percent'] = above_percent
        self.baseline_metrics['between_percent'] = between_percent
        self.baseline_metrics['below_percent'] = below_percent
        
        logger.info("=" * 80)
        logger.info("BASELINE ESTABLISHED")
        logger.info("=" * 80)
        logger.info(f"Threshold: [{threshold_high}, {threshold_low}]")
        logger.info(f"Above: {above_percent}% | Between: {between_percent}% | Below: {below_percent}%")
    ...
    def test_combination(self, threshold_high, threshold_low,
                        above_percent, between_percent, below_percent):
        """Test a specific stop loss parameter combination"""
        logger.info(f"Testing combination: Threshold=[{threshold_high}, {threshold_low}], "
                   f"Above={above_percent}%, Between={between_percent}%, Below={below_percent}%")
        
        # Update backtesting config
        if not self.update_stop_loss_parameters(
            threshold_high, threshold_low,
            above_percent, between_percent, below_percent
        ):
            logger.error("Failed to update backtesting config")
            return None
        ...
        metrics['threshold_low'] = threshold_low
        metrics['above_percent'] = above_percent
        metrics['between_percent'] = between_percent
        metrics['below_percent'] = below_percent
    ...
    def _generate_range(self, min_val, max_val, step):
        """Generate range list, handling float steps"""
        if step == 0 or min_val == max_val:
            return [min_val]
        
        result = []
        current = min_val
        while current <= max_val + (step / 2):  # Add small epsilon for float comparison
    ...
    def generate_all_combinations(self):
        ...
        all_combinations = []
        for th_high in threshold_highs:
            for th_low in threshold_lows:
                # Ensure high threshold > low threshold
                if th_high <= th_low:
                    continue
                for above_pct in above_percents:
                    for between_pct in between_percents:
                        for below_pct in below_percents:
                            all_combinations.append((
                                th_high, th_low,
                                above_pct, between_pct, below_pct
                            ))
    ...
    def run_test_combinations(self, num_combinations=4):
        ...
        for i, (th_high, th_low, above_pct, between_pct, below_pct) in enumerate(test_combinations, 1):
    ...
    def run_full_grid_search(self):
        """Run full grid search across all stop loss parameter combinations"""
        logger.info("Starting full grid search with all stop loss parameter combinations...")
        ...
        for i, (th_high, th_low, above_pct, between_pct, below_pct) in enumerate(all_combinations, 1):
            iteration_start = time.time()
    ...
    def save_results(self, results, filename="grid_search_results.json"):
        ...
        for result in results:
            ...
            for key, value in result.items():
                ...
                # Keep integer values as-is (trades, thresholds)
    ...
    def print_summary(self, results):
        ...
        logger.info("=" * 120)
        logger.info(f"{'Th_High':<8} {'Th_Low':<8} {'Above%':<8} {'Between%':<9} {'Below%':<8} "
        ...
        for i, result in enumerate(sorted_results[:10], 1):
            ...
            logger.info(f"{result['threshold_high']:<8} {result['threshold_low']:<8} "
                       f"{result['above_percent']:<8} {result['between_percent']:<9} {result['below_percent']:<8} "
                       f"{pnl_str:<18} "
        ...
        if best:
            logger.info(f"BEST COMBINATION:")
            logger.info(f"  Threshold: [{best['threshold_high']}, {best['threshold_low']}]")
            logger.info(f"  Above: {best['above_percent']}% | Between: {best['between_percent']}% | Below: {best['below_percent']}%")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Stop Loss Grid Search Tool'
    )
    parser.add_argument(
        '--test',
        action='store_true',
    ...
    try:
        ...
        grid_search = StopLossGridSearch(config_path=args.config)
        
        # Backup backtesting config
        grid_search.backup_backtesting_config()
        ...
        try:
            ...
            if not args.skip_baseline:
                ...
                if grid_search.baseline_metrics:
                    config = grid_search.load_backtesting_config()
                    entry2_config = config.get('ENTRY2', {}) if config else {}
                    threshold_list = entry2_config.get('STOP_LOSS_PRICE_THRESHOLD', [120, 70])
                    sl_percent = entry2_config.get('STOP_LOSS_PERCENT', {})
                    grid_search.baseline_metrics['threshold_high'] = threshold_list[0] if len(threshold_list) > 0 else 120
                    grid_search.baseline_metrics['threshold_low'] = threshold_list[1] if len(threshold_list) > 1 else 70
                    grid_search.baseline_metrics['above_percent'] = sl_percent.get('ABOVE_THRESHOLD', 6.0)
                    grid_search.baseline_metrics['between_percent'] = sl_percent.get('BETWEEN_THRESHOLD', 7.5)
                    grid_search.baseline_metrics['below_percent'] = sl_percent.get('BELOW_THRESHOLD', 8.0)
            ...
            restore_success = grid_search.restore_backtesting_config()
