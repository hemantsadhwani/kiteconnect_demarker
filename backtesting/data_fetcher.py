...
class DataFetcher:
    ...
    def _load_config(self):
        ...
        try:
            ...
            # data_config.yaml STRIKE_MODE is the authority for data collection.
            # data_fetcher.py is always run standalone — backtesting_config.yaml
            # does NOT override here (data collection is mode-independent of backtest).
            config = resolve_strike_mode(config)
            logger.info(f"Configuration loaded from {self.config_path} (STRIKE_MODE: {config.get('STRIKE_MODE', 'ST50')})")
            return config
