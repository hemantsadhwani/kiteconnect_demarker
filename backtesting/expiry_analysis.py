...
class EnhancedExpiryAnalysis:
    def __init__(self):
        ...
        self.data_dir = self.base_dir / "data_st50"
        self.config = None
    ...
    def load_config(self):
        ...
        try:
            ...
            self.config = resolve_strike_mode(self.config)
            self.data_dir = self.base_dir / self.config.get('PATHS', {}).get('DATA_DIR', 'data_st50')
    ...
    def discover_expiry_weeks(self):
        ...
        def parse_expiry_week(expiry_week):
            ...
            if match:
                ...
                day = int(day_str)
                # 2026 for JAN/MAR; MAR above FEB in sort; 2025 for APR–DEC
    ...
    def discover_trading_days(self, expiry_week):
        ...
        expected_days = None
        if self.config:
            cpr_config_path = self.base_dir / 'grid_search_tools' / 'cpr_market_sentiment_v1' / 'config.yaml'
    ...
    def calculate_pnl_metrics(self, df):
        ...
        if pnl_col is None:
            # Only log as debug, not warning, since this is expected for some file types
...
<body>
    ...
    <script>
        ...
        function getYearForExpiry(expiryWeek) {
            ...
            if (match) { const month = monthMap[match[1]] || 0; return month <= 3 ? 2026 : 2025; }
            return '';
        }
