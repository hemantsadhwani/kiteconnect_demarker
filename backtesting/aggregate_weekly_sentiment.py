...
def collect_all_filtered_days(base_path, config=None):
    ...
    """
    filtered_days = []
    data_dir_name = config.get('PATHS', {}).get('DATA_DIR', 'data_st50') if config else 'data_st50'
    data_path = Path(base_path) / data_dir_name
...
def find_sentiment_files(base_path, analysis_config=None, entry_type='Entry2', global_filtered_days=None):
    ...
    data_path = get_data_dir(Path(base_path))
