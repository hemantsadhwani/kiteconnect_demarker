"""
Report DYNAMIC_OTM trades by CPR zones: Below S1, Above R1, Between R1 and S1.

For each zone: number of trades, wins, win rate, total PnL (%).
Uses entry2_dynamic_otm_mkt_sentiment_trades.csv only (DYNAMIC_OTM).
...
def main() -> None:
    ...
    for date_str, trades_path in pairs:
        ...
        for i, row in trades_df.iterrows():
            total_trades_all += 1
            # DYNAMIC_OTM only: skip if strike_type column exists and is not DYNAMIC_OTM
            ...
            if is_high_zero_or_missing(raw_high):
                try:
                    ...
                    if entry_price is not None and pd.notna(entry_price):
                        ...
                        strategy_file = data_dir / "OTM" / f"{plain_symbol}_strategy.csv"
                        if not strategy_file.exists():
                            strategy_file = data_dir / "otm" / f"{plain_symbol}_strategy.csv"
    ...
    # Report — DYNAMIC_OTM only; zone stats and CSVs include EXECUTED trades only (exclude SKIPPED)
    print(f"Total trades in files: {total_trades_all}")
    print(f"DYNAMIC_OTM trades considered: {total_dynamic_otm}")
    ...
    # Save one Excel file with one sheet per zone (analytics folder)
    out_dir = script_dir
    ...
    out_xlsx = out_dir / "trades_dynamic_otm_by_cpr_zone.xlsx"
    zone_sheets = [
        ("Between R1 and S1", between_r1_s1_trades),
        ("Below S3", below_s3_trades),
        ("Between S2 and S3", between_s2_s3_trades),
        ("Between R2 and R3", between_r2_r3_trades),
        ("Above R3", above_r3_trades),
        ("Between S1 and S2", s2_to_s1_trades),
        ("Between R1 and R2", r1_to_r2_trades),
    ]
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        for sheet_name, trade_list in zone_sheets:
            df = pd.DataFrame(trade_list) if trade_list else empty_df
            df.to_excel(writer, sheet_name=sheet_name, index=False)
    print(f"Saved {out_xlsx.name} with {len(zone_sheets)} sheets:")
    for sheet_name, trade_list in zone_sheets:
        print(f"  {sheet_name}: {len(trade_list)} trades")
    print()
