...
CPR (R1, S1, R2, S2) from cpr_dates.csv only (daily OHLC from Kite API; no 1min-derived CPR).
Uses analytics/trade_analytics_by_cpr_band/cpr_dates.csv for all days.
...
# Silence pandas "Parsing dates in %Y-%m-%d format when dayfirst=True was specified" warnings
warnings.filterwarnings("ignore", message=".*dayfirst.*", category=UserWarning)
...
def _normalize_date_key(s: str) -> str:
    """Parse date string to YYYY-MM-DD. Prefer ISO format to avoid pandas dayfirst warning."""
    s = str(s).strip()
    try:
...
def nifty_price_at_time(nifty_df: pd.DataFrame, entry_time_parsed) -> Optional[float]:
    ...
    # Normalize time to naive for comparison (nifty date may be timezone-aware, so dt.time has tz)
    time_col = df["time"].apply(lambda x: x.replace(tzinfo=None) if hasattr(x, "tzinfo") and x.tzinfo else x)
    row = df[time_col == t_candle]
    if row.empty:
...
def get_pnl_series(df: pd.DataFrame) -> pd.Series:
    """Prefer sentiment_pnl (mkt_sentiment after trailing stop), then realized_pnl_pct, then pnl."""
...
def main() -> None:
    ...
    if cpr_csv_path.exists():
        try:
            ...
            if "date" in cpr_df.columns and "R1" in cpr_df.columns and "S1" in cpr_df.columns and "R3" in cpr_df.columns and "S3" in cpr_df.columns:
                for _, row in cpr_df.iterrows():
                    try:
                        d = _normalize_date_key(row["date"])
                        cpr_csv_by_date[d] = {"R1": float(row["R1"]), "S1": float(row["S1"]), "R2": float(row["R2"]), "S2": float(row["S2"]), "R3": float(row["R3"]), "S3": float(row["S3"])}
                    except Exception as row_e:
                        logger.debug("Skip row in cpr_dates.csv (date=%s): %s", row.get("date"), row_e)
                logger.info("Loaded cpr_dates.csv from %s (%d dates)", cpr_csv_path.name, len(cpr_csv_by_date))
        except Exception as e:
            logger.warning("Could not load cpr_dates.csv from %s: %s", cpr_csv_path, e)
    if not cpr_csv_by_date:
        logger.error("cpr_dates.csv not found or empty at %s; cannot proceed", cpr_csv_path)
        sys.exit(1)
    ...
    # Extended zones (R2, S2, R3, S3) — also keep trade lists for CSV export
    below_s3_pnl: List[float] = []
    below_s3_wins: List[bool] = []
    below_s3_trades: List[Dict] = []
    between_s2_s3_pnl: List[float] = []
    between_s2_s3_wins: List[bool] = []
    ...
    between_r2_r3_pnl: List[float] = []
    between_r2_r3_wins: List[bool] = []
    between_r2_r3_trades: List[Dict] = []
    above_r3_pnl: List[float] = []
    above_r3_wins: List[bool] = []
    above_r3_trades: List[Dict] = []
    skipped_dates: List[Tuple[str, str]] = []  # (date, reason)
    total_trades_all = 0
    total_dynamic_atm = 0
    total_with_nifty_and_pnl = 0
    total_executed_with_nifty_pnl = 0  # EXECUTED only (excludes SKIPPED) — "trades after filters"
    # Store first 3 dates' CPR (from cpr_dates.csv) for optional display
    cpr_sample: List[Tuple[str, Dict[str, float]]] = []
    ...
    for date_str, trades_path in pairs:
        ...
        if not nifty_file.exists():
            ...
            continue
        # Use CPR from cpr_dates.csv only (daily OHLC from Kite; no 1min-derived CPR)
        date_norm = _normalize_date_key(date_str)
        cpr_row_key = None
        if date_norm in cpr_csv_by_date:
            cpr_row_key = date_norm
        # Fallback: match by parsed date (handles CSV keys that differ by format but same calendar date)
        if cpr_row_key is None:
            target_d = pd.to_datetime(_normalize_date_key(date_str)).date()
            for k in cpr_csv_by_date:
                try:
                    if pd.to_datetime(_normalize_date_key(k)).date() == target_d:
                        cpr_row_key = k
                        break
                except Exception:
                    continue
        if cpr_row_key is None:
            try:
                normalized = [_normalize_date_key(d) for d in all_days]
                sorted_dates = sorted(set(normalized))
        ...
        if cpr_row_key is None:
            ...
            for _ in range(10):
                ...
                key = str(dt)
                if key in cpr_csv_by_date:
                    cpr_row_key = key
                    break
        if cpr_row_key is None or cpr_row_key not in cpr_csv_by_date:
            skipped_dates.append((date_str, "date not in cpr_dates.csv"))
            continue
        cpr = dict(cpr_csv_by_date[cpr_row_key])
        if "PIVOT" not in cpr:
            cpr["PIVOT"] = (cpr["R1"] + cpr["S1"]) / 2
        _validate_cpr_order(cpr, date_str)
        if len(cpr_sample) < 3:
            cpr_sample.append((date_str, {"R1": cpr["R1"], "S1": cpr["S1"], "R2": cpr["R2"], "S2": cpr["S2"], "R3": cpr["R3"], "S3": cpr["S3"]}))
        r1, s1 = cpr["R1"], cpr["S1"]
        r2, s2 = cpr["R2"], cpr["S2"]
        r3, s3 = cpr["R3"], cpr["S3"]
        try:
        ...
        for i, row in trades_df.iterrows():
            ...
            if "SKIPPED" in trade_status.upper():
                continue
            total_executed_with_nifty_pnl += 1
            ...
            # HYPERLINKs relative to this CSV's folder (trade_analytics_by_cpr_band): ../../data/EXPIRY_DYNAMIC/DAY/ATM/
            ...
            if nifty_at_entry < s1:
                ...
                if nifty_at_entry <= s3:
                    below_s3_pnl.append(float(pnl_val))
                    below_s3_wins.append(float(pnl_val) > 0)
                    below_s3_trades.append(rec)
                elif nifty_at_entry <= s2:
                    between_s2_s3_pnl.append(float(pnl_val))
                    between_s2_s3_wins.append(float(pnl_val) > 0)
                ...
                if nifty_at_entry >= r3:
                    above_r3_pnl.append(float(pnl_val))
                    above_r3_wins.append(float(pnl_val) > 0)
                    above_r3_trades.append(rec)
                elif nifty_at_entry >= r2:
                    between_r2_r3_pnl.append(float(pnl_val))
                    between_r2_r3_wins.append(float(pnl_val) > 0)
    ...
    # CPR source: cpr_dates.csv only (sample for first 3 dates)
    if cpr_sample:
        print("CPR from cpr_dates.csv (sample):")
        for date_str, cpr_vals in cpr_sample:
            print(f"  {date_str}: R1={cpr_vals['R1']:.2f} S1={cpr_vals['S1']:.2f} R2={cpr_vals['R2']:.2f} S2={cpr_vals['S2']:.2f} R3={cpr_vals['R3']:.2f} S3={cpr_vals['S3']:.2f}")
        print()

    # Report — DYNAMIC_ATM only; zone stats and CSVs include EXECUTED trades only (exclude SKIPPED)
    print(f"Total trades in files: {total_trades_all}")
    print(f"DYNAMIC_ATM trades considered: {total_dynamic_atm}")
    print(f"Total trades after filters (EXECUTED only): {total_executed_with_nifty_pnl}")
    print(f"  -> Below S1: {len(below_s1_pnl)}  |  Above R1: {len(above_r1_pnl)}  |  Between R1 and S1: {len(between_r1_s1_pnl)}")
    print(f"  -> Below S3: {len(below_s3_pnl)}  |  S2-S3: {len(between_s2_s3_pnl)}  |  S1-S2: {len(s2_to_s1_pnl)}  |  R1-R2: {len(r1_to_r2_pnl)}  |  R2-R3: {len(between_r2_r3_pnl)}  |  Above R3: {len(above_r3_pnl)}\n")
    ...
    print("\n--- DYNAMIC_ATM: Trades between S2 and S3 (S3 < Nifty at entry <= S2) ---")
    _zone_summary("Between S2 and S3", len(between_s2_s3_pnl), between_s2_s3_wins, between_s2_s3_pnl)

    print("\n--- DYNAMIC_ATM: Trades below S3 (Nifty at entry <= S3) ---")
    _zone_summary("Below S3", len(below_s3_pnl), below_s3_wins, below_s3_pnl)
    ...
    print("\n--- DYNAMIC_ATM: Trades between R2 and R3 (R2 <= Nifty at entry < R3) ---")
    _zone_summary("Between R2 and R3", len(between_r2_r3_pnl), between_r2_r3_wins, between_r2_r3_pnl)

    print("\n--- DYNAMIC_ATM: Trades above R3 (Nifty at entry >= R3) ---")
    _zone_summary("Above R3", len(above_r3_pnl), above_r3_wins, above_r3_pnl)
    ...
    out_dir = script_dir
    sample = between_r1_s1_trades or below_s1_trades or above_r1_trades or s2_to_s1_trades or r1_to_r2_trades or below_s3_trades or between_s2_s3_trades or between_r2_r3_trades or above_r3_trades
    empty_df = pd.DataFrame(columns=list(sample[0].keys())) if sample else pd.DataFrame()

    # Only write mutually exclusive zone CSVs (below_s1 / above_r1 are aggregates of these)
    ...
    out_below_s3 = out_dir / "trades_dynamic_atm_below_s3.csv"
    (pd.DataFrame(below_s3_trades) if below_s3_trades else empty_df).to_csv(out_below_s3, index=False)
    print(f"Saved {len(below_s3_trades)} Below S3 trades to {out_below_s3}")
    out_between_s2_s3 = out_dir / "trades_dynamic_atm_between_s2_s3.csv"
    (pd.DataFrame(between_s2_s3_trades) if between_s2_s3_trades else empty_df).to_csv(out_between_s2_s3, index=False)
    print(f"Saved {len(between_s2_s3_trades)} Between S2 and S3 trades to {out_between_s2_s3}")
    out_between_r2_r3 = out_dir / "trades_dynamic_atm_between_r2_r3.csv"
    (pd.DataFrame(between_r2_r3_trades) if between_r2_r3_trades else empty_df).to_csv(out_between_r2_r3, index=False)
    print(f"Saved {len(between_r2_r3_trades)} Between R2 and R3 trades to {out_between_r2_r3}")
    out_above_r3 = out_dir / "trades_dynamic_atm_above_r3.csv"
    (pd.DataFrame(above_r3_trades) if above_r3_trades else empty_df).to_csv(out_above_r3, index=False)
    print(f"Saved {len(above_r3_trades)} Above R3 trades to {out_above_r3}")
