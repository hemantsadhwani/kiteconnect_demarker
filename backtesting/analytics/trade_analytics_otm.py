#!/usr/bin/env python3
"""
Comprehensive Trade Analytics for OTM (Out-of-The-Money) trades.
Analyzes filtered Entry2 dynamic OTM trades and provides detailed statistics and insights.
Uses entry2_dynamic_otm_mkt_sentiment_trades.csv (filtered) and entry2_dynamic_otm_ce/pe_trades.csv (unfiltered).
"""
...
def find_all_filtered_trade_files(
    ...
    if allowed_day_dirs:
        for day_dir in allowed_day_dirs:
            file_path = day_dir / f'{entry_type_lower}_dynamic_otm_mkt_sentiment_trades.csv'
            if file_path.exists():
                trade_files.append(file_path)
            else:
                logger.debug(f"Missing filtered OTM trades file in {day_dir}")
    else:
        # Look for dynamic OTM market sentiment trades
        for file_path in data_dir.rglob(f'{entry_type_lower}_dynamic_otm_mkt_sentiment_trades.csv'):
            trade_files.append(file_path)
...
def find_unfiltered_trades(
    ...
    if allowed_day_dirs:
        for day_dir in allowed_day_dirs:
            ce_path = day_dir / f'{entry_type_lower}_dynamic_otm_ce_trades.csv'
            pe_path = day_dir / f'{entry_type_lower}_dynamic_otm_pe_trades.csv'
            if ce_path.exists():
                trade_files.append(ce_path)
            if pe_path.exists():
                trade_files.append(pe_path)
            if not ce_path.exists() and not pe_path.exists():
                logger.debug(f"No OTM CE/PE trade files found in {day_dir}")
    else:
        # Look for dynamic OTM CE and PE trades (before sentiment filtering)
        for file_path in data_dir.rglob(f'{entry_type_lower}_dynamic_otm_ce_trades.csv'):
            trade_files.append(file_path)
        for file_path in data_dir.rglob(f'{entry_type_lower}_dynamic_otm_pe_trades.csv'):
            trade_files.append(file_path)
...
def determine_allowed_day_dirs(
    ...
    summary_paths = sentiment_files.get('DYNAMIC_OTM', [])
    if not summary_paths:
        logger.debug("No DYNAMIC_OTM sentiment files returned from discovery")
        return None
    
    allowed_dirs = [path.parent for path in summary_paths]
    logger.info(f"Applying CPR-filtered day selection for OTM: {len(allowed_dirs)} days retained")
    return allowed_dirs
...
def get_price_slab(entry_price):
    """Get price slab category for entry price"""
    try:
        ...
        if price > 160:
            ...
            return None  # Price < 20, not in requested slabs
...
def calculate_pnl_ranges(df):
    ...
    if pnl_series.dtype == 'object':
        pnl_series = pnl_series.str.replace('%', '').str.strip()
...
def analyze_trades(data_dir: Path = None, entry_type: str = 'Entry2'):
    ...
    logger.info("=" * 100)
    logger.info("ANALYSIS OF FILTERED OTM TRADES")
    logger.info("=" * 100)
    
    # Load filtered OTM trades
    filtered_files = find_all_filtered_trade_files(data_dir, entry_type, allowed_day_dirs)
    if not filtered_files:
        logger.error(f"No filtered OTM trade files found for {entry_type} (looking for entry2_dynamic_otm_mkt_sentiment_trades.csv)")
        return
    
    filtered_df = load_all_trades(filtered_files)
    if filtered_df.empty:
        logger.error("No filtered OTM trades found")
        return
    ...
    if filtered_pnl_col:
        ...
        if pnl_series.dtype == 'object':
            # Handle string values with % and NaN
            pnl_series = pnl_series.str.replace('%', '').str.strip()
            pnl_series = pd.to_numeric(pnl_series, errors='coerce')
        # Fill NaN values with 0 before summing
        filtered_pnl = pnl_series.fillna(0).sum()
    else:
        filtered_pnl = 0
    ...
    if unfiltered_pnl_col:
        ...
        if unfiltered_pnl_series.dtype == 'object':
            # Handle string values with % and NaN
            unfiltered_pnl_series = unfiltered_pnl_series.str.replace('%', '').str.strip()
            unfiltered_pnl_series = pd.to_numeric(unfiltered_pnl_series, errors='coerce')
        # Fill NaN values with 0 before summing
        unfiltered_pnl = unfiltered_pnl_series.fillna(0).sum()
    else:
        unfiltered_pnl = filtered_pnl
    
    pnl_improvement = filtered_pnl - unfiltered_pnl
    
    print(f"\nTotal OTM Trades (before filtering): {total_trades_before}")
    print(f"Filtered OTM Trades (after SENTIMENT + PRICE_ZONES): {filtered_trades}")
            ...
            if len(opt_trades) > 0:
                ...
                if opt_pnl_col:
                    ...
                    if opt_pnl_series.dtype == 'object':
                        opt_pnl_series = opt_pnl_series.str.replace('%', '').str.strip()
                        opt_pnl_series = pd.to_numeric(opt_pnl_series, errors='coerce')
                    opt_pnl_series = opt_pnl_series.fillna(0)
            ...
            if len(sent_trades) > 0:
                ...
                if sent_pnl_col:
                    ...
                    if sent_pnl_series.dtype == 'object':
                        sent_pnl_series = sent_pnl_series.str.replace('%', '').str.strip()
                        sent_pnl_series = pd.to_numeric(sent_pnl_series, errors='coerce')
                    sent_pnl_series = sent_pnl_series.fillna(0)
        ...
        if pnl_col:
            for _, trade in filtered_df.iterrows():
                ...
                if bucket:
                    ...
                    if pd.isna(trade_pnl) or trade_pnl == '':
                        trade_pnl = 0.0
                    elif isinstance(trade_pnl, str):
                        # Remove % and convert to float, handle empty strings
                        trade_pnl_str = trade_pnl.replace('%', '').strip()
                        if trade_pnl_str == '' or trade_pnl_str.lower() == 'nan':
                            trade_pnl = 0.0
                        else:
                            try:
                        ...
                        # Already numeric, but might be NaN
    ...
    print("\n" + "-" * 100)
    print("Win Rate by Entry Price Slabs:")
    print("-" * 100)
    
    if 'entry_price' in filtered_df.columns:
        pnl_col = get_pnl_column_name(filtered_df)
        price_slabs = defaultdict(lambda: {'count': 0, 'pnl': 0.0, 'wins': 0})
        
        if pnl_col:
            for _, trade in filtered_df.iterrows():
                slab = get_price_slab(trade['entry_price'])
                if slab:
                    price_slabs[slab]['count'] += 1
                    trade_pnl = trade[pnl_col]
                    ...
                    price_slabs[slab]['pnl'] += trade_pnl
                    # Track winning trades (PnL > 0)
                    if trade_pnl > 0:
                        price_slabs[slab]['wins'] += 1
        else:
            print("PnL column not found - skipping price slab analysis")
        
        # Sort by price slab order
        slab_order = ["> 160", "100 - 160", "20 - 99"]
        best_win_rate = -1
        best_win_rate_slab = None
        most_trades_slab = 0
        most_trades_slab_name = None
        
        for slab in slab_order:
            if slab in price_slabs:
                data = price_slabs[slab]
                win_rate = (data['wins'] / data['count'] * 100) if data['count'] > 0 else 0.0
                if win_rate > best_win_rate:
                    best_win_rate = win_rate
                    best_win_rate_slab = slab
                if data['count'] > most_trades_slab:
                    most_trades_slab = data['count']
                    most_trades_slab_name = slab
        ...
        for slab in slab_order:
            if slab in price_slabs:
                ...
                win_rate = (data['wins'] / data['count'] * 100) if data['count'] > 0 else 0.0
                markers = []
                if slab == best_win_rate_slab:
                    markers.append(" <-- Best Win Rate")
                if slab == most_trades_slab_name:
                    markers.append(" <-- Most trades")
                
                print(f"Entry Price {slab}: {data['count']} trades, P&L: {data['pnl']:.2f}%, Win Rate: {win_rate:.2f}%{''.join(markers)}")
    else:
        print("\nEntry price column not found in filtered trades")
        ...
        if len(bearish_trades) > 0:
            ...
            if bearish_pnl_col:
                ...
                if bearish_pnl_series.dtype == 'object':
                    bearish_pnl_series = bearish_pnl_series.str.replace('%', '').str.strip()
                    bearish_pnl_series = pd.to_numeric(bearish_pnl_series, errors='coerce')
                bearish_pnl_series = bearish_pnl_series.fillna(0)
        ...
        if len(bullish_trades) > 0:
            ...
            if bullish_pnl_col:
                ...
                if bullish_pnl_series.dtype == 'object':
                    bullish_pnl_series = bullish_pnl_series.str.replace('%', '').str.strip()
                    bullish_pnl_series = pd.to_numeric(bullish_pnl_series, errors='coerce')
                bullish_pnl_series = bullish_pnl_series.fillna(0)
