...
PRICE_BAND_MIN = 20
PRICE_BAND_MAX = 350
# PnL threshold: trades at or below this percentage are considered losing
...
def main():
    ...
    for trade_file, date_str in trade_files:
        ...
        try:
            ...
            if len(df) > 0:
                ...
                logger.info(f"Loaded {len(df)} losing trades from {trade_file.name} (price band {price_min}-{price_max})")
        except Exception as e:
    ...
    logger.info(f"{'='*60}")
    logger.info(f"Total losing trades in price band {price_min}-{price_max}: {len(combined_df)}")
    logger.info(f"Total PnL: {combined_df['pnl'].sum():.2f}")
    logger.info(f"{'='*60}\n")
    
    # Save to Excel with hyperlink preservation
    output_file = output_dir / 'losing_trades.xlsx'
    try:
        ...
        # Process symbol column: resolve OTM/... relative to row's source_trade_file dir (same as workflow CSV location)
        if symbol_col:
        ...
        output_file_csv = output_dir / 'losing_trades.csv'
