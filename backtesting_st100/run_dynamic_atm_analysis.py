...
def _sanitize_slab_strikes(df: pd.DataFrame, strike_diff: int = 100) -> pd.DataFrame:
    """Fix corrupted pe_strike/ce_strike in slabs DataFrame before writing to CSV.
    - Values > 100000 (e.g. 2620299900) -> use last 5 digits.
    - Values in 90000-100000 (e.g. 99800, 99900) -> subtract 75000 to get valid strike.
    - Values in (0, 10000) (weekly format e.g. 200, 300) -> add 25000 to get full strike.
    - Zeros: use other column from same row; if both 0, forward/back fill from neighbors.
    - Enforce PE/CE relationship: PE >= CE and PE - CE <= strike_diff (ATM: CE=floor, PE=ceil).
    """
    if df is None or df.empty or 'pe_strike' not in df.columns or 'ce_strike' not in df.columns:
        return df
    df = df.copy()
    for col in ('pe_strike', 'ce_strike'):
        def _fix(v):
            try:
                v = int(float(v))
            ...
            if 90000 <= v <= 100000:
                return v - 75000
            if 0 < v < 10000:
                return v + 25000  # weekly format -> full strike
            return v
        df[col] = df[col].apply(_fix)
    # Fix zeros: use other column from same row (vectorized to avoid assignment issues)
    pe_zero = (df['pe_strike'] == 0) | (df['pe_strike'].isna())
    ce_zero = (df['ce_strike'] == 0) | (df['ce_strike'].isna())
    df['pe_strike'] = np.where(pe_zero, df['ce_strike'], df['pe_strike'])
    df['ce_strike'] = np.where(ce_zero, df['pe_strike'], df['ce_strike'])
    # Any remaining zeros: fill from neighbors, then default 25000 if still missing
    for col in ('pe_strike', 'ce_strike'):
        bad = (df[col] == 0) | (df[col].isna())
        if bad.any():
            s = df[col].replace(0, np.nan)
            df[col] = s.ffill().bfill().fillna(25000).astype(int)
    # Ensure no zeros or floats remain; coerce to int and fix any remaining 0s
    df['pe_strike'] = df['pe_strike'].astype(int)
    df['ce_strike'] = df['ce_strike'].astype(int)
    still_pe_zero = df['pe_strike'] == 0
    still_ce_zero = df['ce_strike'] == 0
    if still_pe_zero.any():
        df.loc[still_pe_zero, 'pe_strike'] = df.loc[still_pe_zero, 'ce_strike'].values
    if still_ce_zero.any():
        df.loc[still_ce_zero, 'ce_strike'] = df.loc[still_ce_zero, 'pe_strike'].values
    df['pe_strike'] = df['pe_strike'].astype(int)
    df['ce_strike'] = df['ce_strike'].astype(int)
    # Enforce PE >= CE and PE - CE <= strike_diff (ATM: CE=floor(price), PE=ceil(price))
    pe, ce = df['pe_strike'].values, df['ce_strike'].values
    bad_order = pe < ce
    if bad_order.any():
        df.loc[bad_order, 'pe_strike'] = (ce[bad_order] + strike_diff)
    pe = df['pe_strike'].values
    ce = df['ce_strike'].values
    excess_diff = (pe - ce) > strike_diff
    if excess_diff.any():
        df.loc[excess_diff, 'ce_strike'] = (pe[excess_diff] - strike_diff)
    df['pe_strike'] = df['pe_strike'].astype(int)
    df['ce_strike'] = df['ce_strike'].astype(int)
    return df


def _build_merged_slabs_from_nifty(nifty_df: pd.DataFrame, strike_diff: int = 100) -> pd.DataFrame:
    """Build slab periods purely from Nifty 1min: one period per run of same (ce_strike, pe_strike).
    ATM: CE=floor(price/strike_diff)*strike_diff, PE=CE+strike_diff.
    Consecutive minutes with the same (ce, pe) are merged; period end is exclusive (next minute start).
    """
    if nifty_df is None or nifty_df.empty:
        return pd.DataFrame(columns=['start', 'end', 'pe_strike', 'ce_strike'])
    df = nifty_df.copy()
    df['date'] = pd.to_datetime(df['date'])
    if 'nifty_price' not in df.columns:
        df['nifty_price'] = (
            (df['open'] + df['high']) / 2 + (df['low'] + df['close']) / 2
        ) / 2
    df['time_str'] = df['date'].dt.strftime('%H:%M:%S')
    ce = (df['nifty_price'] // strike_diff).astype(int) * strike_diff
    pe = ce + strike_diff
    df['ce_strike'] = ce
    df['pe_strike'] = pe
    # Merge consecutive rows with same (ce_strike, pe_strike)
    periods = []
    run_start = None
    run_ce = None
    run_pe = None
    for _, row in df.iterrows():
        t = row['time_str']
        if run_ce is None or run_ce != row['ce_strike'] or run_pe != row['pe_strike']:
            if run_start is not None:
                # End previous period: end time = last minute + 1 (exclusive)
                end_t = datetime.strptime(prev_t, '%H:%M:%S')
                end_t = (end_t + timedelta(minutes=1)).strftime('%H:%M:%S')
                periods.append({'start': run_start, 'end': end_t, 'pe_strike': int(run_pe), 'ce_strike': int(run_ce)})
            run_start = t
            run_ce = row['ce_strike']
            run_pe = row['pe_strike']
        prev_t = t
    if run_start is not None:
        end_t = datetime.strptime(prev_t, '%H:%M:%S')
        # Last period: if through 15:30, use 15:30:00 as end; else last minute + 1 (exclusive)
        if prev_t == '15:30:00':
            end_t = '15:30:00'
        else:
            end_t = (end_t + timedelta(minutes=1)).strftime('%H:%M:%S')
...
def _correct_slabs_from_nifty(nifty_df: pd.DataFrame, slabs_df: pd.DataFrame, strike_diff: int = 100) -> pd.DataFrame:
    """Correct slab CE/PE from Nifty 1min data when stored strikes are inconsistent with price in that period.
    For each period [start, end), compute expected ATM from mean nifty_price; if stored strikes don't bracket
    the price or pe_strike - ce_strike != strike_diff, overwrite with expected (CE=floor(price/100)*100, PE=CE+100).
    """
    if slabs_df is None or slabs_df.empty or nifty_df is None or nifty_df.empty:
        return slabs_df
    if 'pe_strike' not in slabs_df.columns or 'ce_strike' not in slabs_df.columns:
        return slabs_df
    if 'start' not in slabs_df.columns or 'end' not in slabs_df.columns:
        return slabs_df
    nifty_df = nifty_df.copy()
    nifty_df['date'] = pd.to_datetime(nifty_df['date'])
    if 'nifty_price' not in nifty_df.columns:
        nifty_df['nifty_price'] = (
            (nifty_df['open'] + nifty_df['high']) / 2 +
            (nifty_df['low'] + nifty_df['close']) / 2
        ) / 2
    nifty_df['time_str'] = nifty_df['date'].dt.strftime('%H:%M:%S')
    out = slabs_df.copy()
    for i, row in out.iterrows():
        start_s = str(row['start']).strip()
        end_s = str(row['end']).strip()
        try:
            start_t = datetime.strptime(start_s, '%H:%M:%S').time()
            end_t = datetime.strptime(end_s, '%H:%M:%S').time()
        except Exception:
            continue
        # Rows in [start, end) - for end we use strict < so period is [start, end)
        mask = (
            (nifty_df['time_str'] >= start_s) &
            (nifty_df['time_str'] < end_s)
        )
        period_nifty = nifty_df.loc[mask]
        if period_nifty.empty:
            continue
        mean_price = period_nifty['nifty_price'].mean()
        expected_ce = math.floor(mean_price / strike_diff) * strike_diff
        expected_pe = expected_ce + strike_diff
        try:
            stored_ce = int(float(row['ce_strike']))
            stored_pe = int(float(row['pe_strike']))
        except (TypeError, ValueError):
            continue
        # Correct when: price not in [ce, pe], or difference != strike_diff, or wrong order
        wrong_diff = (stored_pe - stored_ce) != strike_diff
        wrong_order = stored_ce > stored_pe
        price_outside = mean_price < stored_ce or mean_price > stored_pe
        if wrong_diff or wrong_order or price_outside:
            out.at[i, 'ce_strike'] = expected_ce
            out.at[i, 'pe_strike'] = expected_pe
            logger.debug(f"Corrected slab [{start_s} - {end_s}] from ({stored_ce},{stored_pe}) to ({expected_ce},{expected_pe}) (mean_price={mean_price:.2f})")
    out['pe_strike'] = out['pe_strike'].astype(int)
    out['ce_strike'] = out['ce_strike'].astype(int)
    return out
...
class ConsolidatedDynamicATMAnalysis:
        ...
        try:
            ...
            for trade_idx, trade in enumerate(trades_data):
                ...
                if symbol.endswith('PE'):
                    if 'O2025' in symbol:
                        ...
                        match = re.search(r'(\d+)PE$', symbol)
                        strike_str = match.group(1) if match else None
            ...
            for _, row in minute_strikes_df.iterrows():
                if current_pe != row['pe_strike_active'] or current_ce != row['ce_strike_active']:
                    if current_pe is not None:
                        ...
                        # Defensive: corrupted values from symbol parsing - fix before writing to slabs
                        # Values > 100000 (e.g. 2620299900) -> use last 5 digits (29990)
                        # Values in 90000-99999 (e.g. 99800, 99900) are corrupted -> subtract 75000 to get valid strike (24800, 24900)
                        if pe_strike_to_save is not None and isinstance(pe_strike_to_save, (int, float)):
                            v = int(pe_strike_to_save)
                            if v > 100000:
                                v = int(str(v)[-5:])
                            elif 90000 <= v <= 100000:
                                v = v - 75000  # 100000 -> 25000
                            pe_strike_to_save = v
                        if ce_strike_to_save is not None and isinstance(ce_strike_to_save, (int, float)):
                            v = int(ce_strike_to_save)
                            if v > 100000:
                                v = int(str(v)[-5:])
                            elif 90000 <= v <= 100000:
                                v = v - 75000
            ...
            if current_pe is not None:
                ...
                if not is_monthly:
                    ...
                    # Weekly format: strikes are stored as (full_strike - 25000), e.g., 700 instead of 25700
                    # We need to add 25000 back to get full strike
                    if isinstance(pe_strike_to_save, (int, float)) and pe_strike_to_save < 10000:
                        pe_strike_to_save = int(pe_strike_to_save + 25000)
                    if isinstance(ce_strike_to_save, (int, float)) and ce_strike_to_save < 10000:
                        ce_strike_to_save = int(ce_strike_to_save + 25000)
                ...
                if period_start is not None:
                    ...
                    matching_rows = minute_strikes_df[minute_strikes_df['time'] == period_start_time]
                    if not matching_rows.empty:
                        nifty_price = matching_rows['nifty_close'].iloc[0]
                    else:
                        # Fallback 1: find the first row with matching strikes (should be at period_start or later)
                        period_rows = minute_strikes_df[
                            (minute_strikes_df['pe_strike_active'] == current_pe) & 
                        ...
                        if not period_rows.empty:
                            nifty_price = period_rows.iloc[0]['nifty_close']
                        else:
                            # Fallback 2: use the last row with matching strikes before period_start
                            ...
                            if not prev_rows.empty:
                                nifty_price = prev_rows.iloc[-1]['nifty_close']
                            else:
                                # Final fallback: use the last row in minute_strikes_df
                                if not minute_strikes_df.empty:
                                    nifty_price = minute_strikes_df.iloc[-1]['nifty_close']
    ...
    def ensure_nifty_data_and_slabs(self, date_str: str, dest_dir: Path, source_dir: Path = None, expiry_week: str = None):
        ...
        if not slabs_file.exists() or should_regenerate_slabs:
            ...
            strike_diff = self.config['DATA_COLLECTION']['STRIKE_DIFFERENCE']
            # Build slabs from Nifty with merged consecutive same (ce, pe) periods
            slabs_df = _build_merged_slabs_from_nifty(nifty_df, strike_diff=strike_diff)
            if slabs_df.empty:
                logger.error(f"Failed to create dynamic ATM slabs for {date_str}")
                return False
            slabs_df = _sanitize_slab_strikes(slabs_df, strike_diff=strike_diff)
    ...
    def _process_trades_for_entry_type(self, expiry_week: str, day_label: str, entry_type: str, 
        ...
        for strategy_file in strategy_files:
            ...
            for _, entry_signal in entry_signals.iterrows():
                ...
                # CRITICAL FIX: In production, when a candle completes at T, the entry condition evaluation
                # happens when all updates (CE, PE, sentiment) are received, which typically occurs at the start
                # of the next minute (T+1 minute) or shortly after. The trade then executes immediately.
                # Production pattern: Signal detected at candle T -> Evaluation at T+1 minute -> Execution at T+1 minute + ~1 second
                # To match production, we add 1 minute + 1 second to the signal candle time.
                # Note: Production ledger shows 12:53:01 for signal at 12:52:00, so we use 1 minute + 1 second.
                # 
        ...
        for signal in all_global_signals:
            ...
            if signal['type'] == 'entry':
                ...
                for period in all_periods:
                    from datetime import datetime as dt
                    start_time = dt.strptime(period['start'], '%H:%M:%S').time()
                    end_time = dt.strptime(period['end'], '%H:%M:%S').time()
                    
                    # CRITICAL FIX: Period matching - use exclusive end [start, end) so signal at period end belongs to NEXT period
                    # e.g. period 11:23-11:26 and 11:26-11:33: signal at 11:26 must match only 11:26-11:33 (CE=24700), not 11:23-11:26 (CE=24600)
                    # Exception: last period end 15:30:00 is inclusive so 15:30 signal matches
        ...
        if not completed_df.empty:
            ...
            def calculate_metrics(row):
                ...
                if '=HYPERLINK' in str(symbol):
                    ...
                    if match:
                        strategy_file = Path(match.group(1))
                        symbol = strategy_file.stem.replace('_strategy', '')
                else:
                    symbol = str(symbol)
                    strategy_file = source_dir / f"{symbol}_strategy.csv"
    ...
    def run_dynamic_analysis(self, expiry_week: str, day_label: str):
        ...
        # STEP 2: Use Nifty-merged slabs for all_periods so trade matching uses correct CE/PE strikes from slab
        # (e.g. 11:26-11:33 -> CE 24700; only 24700CE matches, not 25000CE)
        if nifty_file.exists():
            nifty_df = pd.read_csv(nifty_file)
            nifty_df['date'] = pd.to_datetime(nifty_df['date'])
            strike_diff = self.config['DATA_COLLECTION']['STRIKE_DIFFERENCE']
            merged_slabs_df = _build_merged_slabs_from_nifty(nifty_df, strike_diff=strike_diff)
            merged_slabs_df = _sanitize_slab_strikes(merged_slabs_df, strike_diff=strike_diff)
            all_periods = []
            for _, row in merged_slabs_df.iterrows():
                try:
                    ...
                    pe_strike = row['pe_strike']
                    ce_strike = row['ce_strike']
                all_periods.append({'start': row['start'], 'end': row['end'], 'pe_strike': pe_strike, 'ce_strike': ce_strike})
            logger.info(f"STEP 2: Using Nifty-merged slabs for all_periods ({len(all_periods)} periods) - trade matching will use correct strikes")
        else:
            logger.warning(f"NIFTY data file not found: {nifty_file}, using initial slab load for all_periods")
        
        # STEP 3: First pass - collect trades with Nifty-merged slabs (correct strikes per period)
        ...
        if all_trades_data and nifty_file.exists():
            ...
            # Write slab CSV from Nifty with merged periods (one period per run of same CE/PE)
            strike_diff = self.config['DATA_COLLECTION']['STRIKE_DIFFERENCE']
            slabs_file = dest_dir / "nifty_dynamic_atm_slabs.csv"
            file_slabs_df = _build_merged_slabs_from_nifty(nifty_df, strike_diff=strike_diff)
            file_slabs_df = _sanitize_slab_strikes(file_slabs_df, strike_diff=strike_diff)
            try:
                file_slabs_df.to_csv(slabs_file, index=False)
                logger.info(f"Saved slabs to: {slabs_file}")
            except PermissionError as e:
                logger.warning(f"Could not write {slabs_file} (file may be open in Excel). Continuing; STEP 5 will use Nifty-merged periods.")
            
            # CRITICAL: Use Nifty-merged periods for trade matching so CE/PE trades use correct strike from slab
            # (e.g. 11:26-11:33 -> CE 24700, not wrong 25000 from blocked slabs)
            all_periods = []
            for _, row in file_slabs_df.iterrows():
                try:
                    pe_strike = int(float(row['pe_strike']))
                    ce_strike = int(float(row['ce_strike']))
                except (ValueError, TypeError):
                    ...
                    ce_strike = row['ce_strike']
                all_periods.append({'start': row['start'], 'end': row['end'], 'pe_strike': pe_strike, 'ce_strike': ce_strike})
            logger.info(f"Using Nifty-merged slabs for trade matching ({len(all_periods)} periods)")
            ...
            try:
                ...
                logger.warning(f"Could not write {blocked_slabs_file} (file may be open). Skipping.")
        else:
        ...
        # STEP 5: Process all trades with Nifty-merged slabs (all_periods from STEP 2 or STEP 4)
