...
def _sanitize_slab_strikes(df: pd.DataFrame, strike_diff: int = 100) -> pd.DataFrame:
    ...
    - Enforce OTM: CE = PE + strike_diff (PE=floor, CE=ceil).
    """
    ...
    df['pe_strike'] = df['pe_strike'].astype(int)
    df['ce_strike'] = df['ce_strike'].astype(int)
    # Enforce OTM: CE = PE + strike_diff (PE=floor(price), CE=ceil(price))
    df['ce_strike'] = (df['pe_strike'] + strike_diff).astype(int)
    return df


def _build_merged_slabs_from_nifty(nifty_df: pd.DataFrame, strike_diff: int = 100) -> pd.DataFrame:
    """Build slab periods purely from Nifty 1min: one period per run of same (pe_strike, ce_strike).
    OTM: PE=floor(price/strike_diff)*strike_diff, CE=PE+strike_diff.
    Consecutive minutes with the same (pe, ce) are merged; period end is exclusive (next minute start).
    """
    ...
    if 'nifty_price' not in df.columns:
        df['nifty_price'] = ((df['open'] + df['high']) / 2 + (df['low'] + df['close']) / 2) / 2
    df['time_str'] = df['date'].dt.strftime('%H:%M:%S')
    pe = (df['nifty_price'] // strike_diff).astype(int) * strike_diff
    ce = pe + strike_diff
    df['pe_strike'] = pe
    df['ce_strike'] = ce
    periods = []
    ...
    for _, row in df.iterrows():
        t = row['time_str']
        if run_pe is None or run_pe != row['pe_strike'] or run_ce != row['ce_strike']:
    ...
    if run_start is not None:
        end_t = datetime.strptime(prev_t, '%H:%M:%S')
        end_t = '15:30:00' if prev_t == '15:30:00' else (end_t + timedelta(minutes=1)).strftime('%H:%M:%S')
        periods.append({'start': run_start, 'end': end_t, 'pe_strike': int(run_pe), 'ce_strike': int(run_ce)})
    return pd.DataFrame(periods)


def _correct_slabs_from_nifty(nifty_df: pd.DataFrame, slabs_df: pd.DataFrame, strike_diff: int = 100) -> pd.DataFrame:
    """Correct slab PE/CE from Nifty 1min when stored strikes are inconsistent with price (OTM: PE=floor, CE=PE+100)."""
    if slabs_df is None or slabs_df.empty or nifty_df is None or nifty_df.empty:
        return slabs_df
    if 'pe_strike' not in slabs_df.columns or 'ce_strike' not in slabs_df.columns or 'start' not in slabs_df.columns or 'end' not in slabs_df.columns:
        return slabs_df
    nifty_df = nifty_df.copy()
    nifty_df['date'] = pd.to_datetime(nifty_df['date'])
    if 'nifty_price' not in nifty_df.columns:
        nifty_df['nifty_price'] = ((nifty_df['open'] + nifty_df['high']) / 2 + (nifty_df['low'] + nifty_df['close']) / 2) / 2
    nifty_df['time_str'] = nifty_df['date'].dt.strftime('%H:%M:%S')
    out = slabs_df.copy()
    for i, row in out.iterrows():
        start_s = str(row['start']).strip()
        end_s = str(row['end']).strip()
        mask = (nifty_df['time_str'] >= start_s) & (nifty_df['time_str'] < end_s)
        period_nifty = nifty_df.loc[mask]
        if period_nifty.empty:
            continue
        mean_price = period_nifty['nifty_price'].mean()
        expected_pe = math.floor(mean_price / strike_diff) * strike_diff
        expected_ce = expected_pe + strike_diff
        try:
            stored_pe = int(float(row['pe_strike']))
            stored_ce = int(float(row['ce_strike']))
        except (TypeError, ValueError):
            continue
        wrong_diff = (stored_ce - stored_pe) != strike_diff
        wrong_order = stored_pe > stored_ce
        price_outside = mean_price < stored_pe or mean_price > stored_ce
        if wrong_diff or wrong_order or price_outside:
            out.at[i, 'pe_strike'] = expected_pe
            out.at[i, 'ce_strike'] = expected_ce
            logger.debug(f"Corrected OTM slab [{start_s} - {end_s}] from ({stored_pe},{stored_ce}) to ({expected_pe},{expected_ce}) (mean_price={mean_price:.2f})")
...
class ConsolidatedDynamicOTMAnalysis:
    ...
    def create_dynamic_otm_slabs(self, nifty_df: pd.DataFrame, date_str: str, source_dir: Path = None, expiry_week: str = None) -> pd.DataFrame:
        try:
            ...
            for _, row in nifty_df.iterrows():
                ...
                timestamp = row['date']
                # OTM logic: PE=floor, CE=ceil (opposite of ATM) - using calculated price instead of close
    ...
    def run_dynamic_analysis(self, expiry_week: str, day_label: str):
        ...
        if all_trades_data and nifty_file.exists():
            ...
            # Write slab CSV from Nifty with merged periods (one period per run of same PE/CE)
            strike_diff = self.config['DATA_COLLECTION']['STRIKE_DIFFERENCE']
            slabs_file = dest_dir / "nifty_dynamic_otm_slabs.csv"
        ...
        if not completed_df.empty:
            ...
            def calculate_metrics(row):
                ...
                if '=HYPERLINK' in str(symbol):
                    ...
                    if match:
                        # Extract the full path from hyperlink
                ...
                try:
                    if strategy_file.exists():
                        ...
                        # Find rows where the time matches entry_time_str (match by hour and minute, ignore seconds)
                        # Strategy files have minute-level times (HH:MM:00), but trade entry times have seconds (HH:MM:01)
                        matching_rows = df_full[(df_full['date'].dt.hour == entry_time_obj.hour) & 
