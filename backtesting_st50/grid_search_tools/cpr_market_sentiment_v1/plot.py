...
def get_swing_bands_from_sentiment_analyzer(csv_file_path, cpr_levels, cpr_band_width=None):
    ...
    try:
        ...
        config = {}
        # Use v1 flag name to control dynamic horizontal S/R from swings
        enable_dynamic_swing_bands = True  # Default to True for backward compatibility
        try:
            with open(config_path, 'r') as f:
                config = yaml.safe_load(f)
                if config:
                    # v1: respect ENABLE_DYNAMIC_HORIZONTAL_BANDS flag (same flag used by analyzer)
                    enable_dynamic_swing_bands = config.get('ENABLE_DYNAMIC_HORIZONTAL_BANDS', True)
