...
def main() -> None:
    ...
    for date_str, trades_path in pairs:
        ...
        if len(cpr_sample) < 3:
            cpr_sample.append((date_str, {"R1": cpr["R1"], "S1": cpr["S1"], "R2": cpr["R2"], "S2": cpr["S2"]}))
        r1, s1 = cpr["R1"], cpr["S1"]
        r2, s2 = cpr["R2"], cpr["S2"]
        try:
    ...
    if cpr_sample:
        ...
        for date_str, cpr_vals in cpr_sample:
            print(f"  {date_str}: R1={cpr_vals['R1']:.2f} S1={cpr_vals['S1']:.2f} R2={cpr_vals['R2']:.2f} S2={cpr_vals['S2']:.2f}")
        print()
