...
Exports all winning trades from a specific price band to an Excel file for analysis
"""
...
PRICE_BAND_MIN = 70
PRICE_BAND_MAX = 249
# PnL threshold: trades at or above this value are considered winning
...
def _build_skip_first_mapping(strategy_file: Path) -> dict:
    """Create mapping of entry_time -> skip_first flag for a strategy file."""
    mapping = {}
...
def add_skip_first_flags(df: pd.DataFrame) -> pd.DataFrame:
    """Add skip_first column indicating first entry after ST switch."""
    if df.empty:
        df['skip_first'] = None
        return df
...
def main():
    ...
    if not all_trades:
        logger.warning("No winning trades found in the specified price band")
        return
    ...
    logger.info(f"{'='*60}")
    logger.info(f"Total winning trades in price band {PRICE_BAND_MIN}-{PRICE_BAND_MAX}: {len(combined_df)}")
