#!/usr/bin/env python3
"""
Create Winning Trades Dataset
Consolidates complete trade cycle from entry to end for each option symbol
Based on sample format from sample_winning_trades_dataset.xlsx
"""
...
def create_dataset():
    """Create the winning trades dataset."""
    # File paths - use relative paths from script location
    script_dir = Path(__file__).parent
    winning_trades_file = script_dir / "winning_trades_70_249.xlsx"
    sample_file = script_dir / "sample_winning_trades_dataset.xlsx"
    output_file = script_dir / "winning_trades_dataset.xlsx"
    
    # Check if files exist
    if not winning_trades_file.exists():
        logger.error(f"Winning trades file not found: {winning_trades_file}")
        return
    
    # Read winning trades - use openpyxl to preserve hyperlinks
    logger.info(f"Reading winning trades from {winning_trades_file}")
    try:
        from openpyxl import load_workbook
        
        # First read with pandas to get data
        df_winning = pd.read_excel(winning_trades_file)
        logger.info(f"Loaded {len(df_winning)} winning trades")
        logger.info(f"Columns in winning trades: {list(df_winning.columns)[:10]}...")
        
        # Check for required columns
        required_cols = ['symbol', 'entry_time', 'exit_time']
        missing_cols = [col for col in required_cols if col not in df_winning.columns]
        if missing_cols:
            logger.error(f"Missing required columns: {missing_cols}")
            logger.error(f"Available columns: {list(df_winning.columns)}")
            return
        ...
        wb = load_workbook(winning_trades_file, read_only=False, keep_links=True)
        ws = wb.active
        ...
        if symbol_col_idx:
            ...
            for row_idx in range(2, len(df_winning) + 2):  # Start from row 2 (skip header)
            ...
            if hyperlink_paths:
                df_winning['symbol_hyperlink'] = df_winning.index.map(hyperlink_paths)
                logger.info(f"✓ Extracted {len(hyperlink_paths)} hyperlinks from {len(df_winning)} rows")
            else:
        ...
        logger.error(f"Error reading winning trades file: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return
    
    # Read sample to understand format (optional, for reference)
    sample_columns = None
    if sample_file.exists():
        try:
            df_sample = pd.read_excel(sample_file)
            sample_columns = list(df_sample.columns)
            logger.info(f"Sample file has {len(sample_columns)} columns: {sample_columns[:10]}...")
        except Exception as e:
    ...
    has_source_file_col = 'source_trade_file' in df_winning.columns
    has_date_col = 'date' in df_winning.columns
    
    logger.info(f"Columns available: source_trade_file={has_source_file_col}, date={has_date_col}")
    logger.info(f"Processing {len(df_winning)} trades from input file...")
    
    for idx, row in df_winning.iterrows():
        logger.debug(f"Processing row {idx} (trade {len(trade_cycles_dict) + 1}/{len(df_winning)})")
        ...
        if symbol_hyperlink:
            ...
            if strategy_file.exists():
                ...
                logger.info(f"Row {idx+1}/{len(df_winning)}: ✓ Found strategy file via hyperlink: {strategy_file.name}")
            else:
        ...
        # Extract trade cycle
        logger.info(f"Processing trade {idx+1}/{len(df_winning)}: {symbol_name}")
    ...
    all_input_indices = list(df_winning.index)
    ...
    if missing_indices:
        ...
        for missing_idx in missing_indices:
            row = df_winning.loc[missing_idx]
    ...
    if not all_trade_cycles:
        ...
        logger.error(f"Processed {len(df_winning)} trades, but created 0 trade cycles (including placeholders).")
        return
    
    # Final verification
    if len(all_trade_cycles) != len(df_winning):
        logger.error(f"⚠️  CRITICAL ERROR: Trade count still mismatched!")
        logger.error(f"   Expected: {len(df_winning)} trades")
        logger.error(f"   Got: {len(all_trade_cycles)} trades")
        return
    else:
        logger.info(f"✅ Successfully processed all {len(all_trade_cycles)} trade cycles (matches input: {len(df_winning)})")
    ...
    # Final check: We MUST have exactly len(df_winning) trade cycles before combining
    if len(aligned_cycles) != len(df_winning):
        logger.error(f"⚠️  CRITICAL: Before combining, we have {len(aligned_cycles)} cycles but need {len(df_winning)}")
    ...
    logger.info(f"Combining {len(aligned_cycles)} trade cycles (expected {len(df_winning)})")
    combined_rows = []
    ...
    logger.info(f"Final dataset: {len(final_df)} total rows, {unique_symbols} unique symbols (expected {len(df_winning)})")
    
    if unique_symbols != len(df_winning):
        logger.error(f"⚠️  CRITICAL WARNING: Unique symbol count ({unique_symbols}) doesn't match input ({len(df_winning)})")
        logger.error(f"   This means {len(df_winning) - unique_symbols} trades are missing from the output!")
        
        # Get input symbols
        input_symbols = set(str(s) for s in df_winning['symbol'].unique() if pd.notna(s))
        ...
        for missing_sym in missing_symbols:
            ...
            matching_rows = df_winning[df_winning['symbol'].astype(str) == str(missing_sym)]
        ...
        non_blank_final = final_df[final_df['symbol'].notna() & (final_df['symbol'].astype(str).str.strip() != '')]
        unique_symbols = non_blank_final['symbol'].nunique()
        logger.info(f"   After fix: {unique_symbols} unique symbols (expected {len(df_winning)})")
    ...
    try:
        ...
        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            final_df.to_excel(writer, sheet_name='Winning Trades Dataset', index=False)
...
if __name__ == '__main__':
    ...
    log_file = Path(__file__).parent / 'create_dataset.log'
