...
def main() -> None:
    ...
    def report(name: str, df: pd.DataFrame) -> None:
        ...
        # Trades that would have hit 7% or 10% TP (high >= TP)
        hit_7 = (high >= 7).sum()
        hit_10 = (high >= 10).sum()
        valid_high = high.notna()
        print()
        print(f"  Trades with high >= 7%:  {int(hit_7)} (of {valid_high.sum()} with valid high)")
        print(f"  Trades with high >= 10%: {int(hit_10)} (of {valid_high.sum()} with valid high)")
        print()
