...
def apply_trailing_stop(csv_path: Path, config_path: Path, output_path: Path = None):
    ...
    for idx, row in df.iterrows():
        ...
        # If executing this trade would breach the limit, SKIP it (do not execute) and stop the day.
        # This prevents any trade from being executed after the high-water-mark breach.
        if projected_capital < drawdown_limit:
            ...
            df.at[idx, 'trade_status'] = 'SKIPPED (RISK STOP)'
            df.at[idx, 'trade_status_reason'] = (
                f'MARK2MARKET: Trade skipped; executing this trade would have breached drawdown limit '
                f'(capital would fall below {loss_mark}% from day high); no further trades for the day'
            )
            if 'sentiment_pnl' in df.columns:
                df.at[idx, 'sentiment_pnl'] = 0
            logger.warning(
                f"Trade {idx+1}: Skipped (would breach). Projected capital {projected_capital:,.2f} < "
                f"Limit {drawdown_limit:,.2f}. Stopping all further trades for the day."
            )
            continue
