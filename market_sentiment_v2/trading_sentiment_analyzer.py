...
class TradingSentimentAnalyzer:
    ...
    def process_new_candle(self, candle_or_ohlc: dict, timestamp: str = None) -> dict:
        """
        Main entry point. Processes a single new candle.
        
        Compatible with both signatures:
        - Backtesting: process_new_candle(candle: dict) where candle has 'date' field
        - Production: process_new_candle(ohlc: dict, timestamp: str = None)
        """
        # Convert to standard candle format
        if 'date' in candle_or_ohlc:
            candle = candle_or_ohlc.copy()
        elif timestamp:
            candle = candle_or_ohlc.copy()
            candle['date'] = timestamp
        else:
            candle = candle_or_ohlc.copy()
            candle['date'] = f"candle_{self.current_candle_index + 1}"
    ...
    def _handle_new_swing(self, price, swing_type, timestamp=""):
        ...
        for name, level in self.cpr_levels.items():
            ...
            if bearish_zone[0] <= price <= bearish_zone[1]:
                ...
                return {'neutralized_at': swing_detection_idx, 'level_name': name, 'zone_type': 'bearish'}  # Do not create horizontal band
    ...
    def get_current_sentiment(self) -> str:
        """Returns the current sentiment. Compatible with v1 interface."""
